...
use App\{Models\User, Models\Message, Models\Product, Models\Category, Classes\GeniusMailer, Models\Conversation};
...
class VendorController extends FrontBaseController
{
    public function index(Request $request, $slug)
    {
        ...
        if ($search) {
            $categories = Category::where('name','LIKE', '%'.$search.'%')->orWhere('slug','LIKE', '%'.$search.'%')->get();
        } else {
            $categories = Category::where('language_id', $langg->id)
            ->where('status', 1)
            ->get();
        }
    ...
    public function categorySearch(Request $request){
        $search = $request->search;
        $categories = Category::where('name',$search)->orWhere('slug',$search)->get();
