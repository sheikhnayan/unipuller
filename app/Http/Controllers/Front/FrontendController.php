...
// CURRENCY SECTION ENDS
	...
	public function vendorList(Request $request,$slug = null, $slug1 = null)
	{
        ...
        $vendors = UserShop::orderby('id','desc')->when($category, function ($query, $category) {
            if(!empty($category)){
	...
	public function vendorService($id)
	{
        ...
        $shops = UserShop::where('user_id',$id)->with('services')->get();
	...
	{
        $shop = UserShop::where('id',$id)->with('services','products')->first();
    ...
    public function serviceReviewsubmit(Request $request)
    {
        ...
        {
            $user = Auth::user();
            $prev_reviewer = Rating::where('service_id','=',$request->service_id)->where('user_id','=',$user->id)->first();
            ...
            return response()->json(array('errors' => [ 0 => __('Take This Service First') ]));
        }
    }
...
// ------------------ Rating SECTION ENDS --------------------
    ...
    public function autosearch($slug)
    {
        if(mb_strlen($slug,'UTF-8') > 1){
            ...
            $sers = UserService::where('name', 'like', '%' . $search . '%')->orWhere('name', 'like', $slug . '%')->where('status','=',1)->orderby('id','desc')->take(10)->get();
    ...
    public function autoshopsearch($slug)
    {
        ...
        if(mb_strlen($slug,'UTF-8') > 1){
            $search = ' '.$slug;
            $prods = UserShop::where('shop_name', 'like', '%' . $search . '%')->orWhere('shop_name', 'like', $slug . '%')->orWhere('shop_address', 'like', $slug . '%')->orWhere('shop_address', 'like', '%' . $search . '%')->where('status','=',1)->orderby('id','desc')->take(10)->get();
